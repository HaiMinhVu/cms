...
$(document).ready(function(){
	...
	$('#category_modal').on('hidden.bs.modal', function(e){
		window.EventBus.$emit('category-selected-default', null);
	});

	// double click on node to update
	$('#cat_tree').bind("select_node.jstree", function (event, data) {
		...
		$.ajax({
			...
			success:function(mess){
				...
				window.EventBus.$emit('category-selected-default', parseInt(catObj.parent));
				$('#category_form')[0].reset();
				$('#catParentID').val(catObj.parent);
		...
		if(selected.length == 0){
			...
			$.ajax({
				...
				success:function(mess){
					...
					$('#category_form')[0].reset();
					// console.log(catObj)
					// $('#catParentID').value(catObj.parent);
	...
	window.EventBus.$on('category-selected', (categoryId) => {
